...
import { classifyFieldType } from "./_classifyFieldType.js";
...
export function runValidateSerializable(opts) {
...
function visitSourceFile(sf, checker, out) {
...
function processClass(cls, sf, checker, out) {
    ...
    for (const member of cls.members) {
        ...
        if (!propertyCall) {
            out.push(diag(sf, member, className, propName, "MISSING_PROPERTY_DECORATOR", "error", `Field "${propName}" is missing @property() — the serializer will not see it.`));
            continue;
        }
...
function emitClassificationDiagnostics(c, declaredTypes, sf, member, className, propName, inCollection, out) {
    switch (c.kind) {
        case "basic":
        case "missingTypeAnnotation":
            return;
        case "classWithoutJsonTypeName": {
            const code = inCollection ? "MISSING_TYPE_ARG_IN_COLLECTION" : "MISSING_TYPE_ARG";
            const where = inCollection ? `collection element class ${c.symbolName}` : `class ${c.symbolName}`;
            out.push(diag(sf, member, className, propName, code, "error", `Field "${propName}" has ${where} without jsonTypeName — @property(${c.symbolName}) is required.`));
            return;
        }
        case "classWithJsonTypeName": {
            ...
            const code = inCollection ? "TYPE_NOT_IN_TYPES_LIST_IN_COLLECTION" : "TYPE_NOT_IN_TYPES_LIST";
            out.push(diag(sf, member, className, propName, code, "error", `Field "${propName}" uses class ${c.symbolName} (with jsonTypeName) — add it to @serializable({types: [${c.symbolName}, ...]}).`));
            return;
        }
        case "collection":
        ...
        case "union": {
            ...
            out.push(diag(sf, member, className, propName, "UNION_TYPE_AMBIGUOUS", severity, `Field "${propName}" is a union of multiple class types — provide an explicit @property(SerializerOrClass) to disambiguate.`));
            return;
        }
    }
}
function isSkippedField(member) {
...
function findPropertyDecoratorCall(member) {
...
function hasSerializableDecorator(cls) {
...
function diag(sf, node, className, propName, code, severity, message) {
...
//# sourceMappingURL=_runValidateSerializable.js.map