...
export type FieldClassification =
  ...
  | {kind: "basic"}
  | {kind: "classWithJsonTypeName"; symbol: ts.Symbol; symbolName: string}
  | {kind: "classWithoutJsonTypeName"; symbol: ts.Symbol; symbolName: string}
  | {kind: "collection"; container: "Array" | "Set" | "Map"; inner: FieldClassification}
  | {kind: "union"; branches: FieldClassification[]};

const BASIC_TYPE_NAMES = new Set<string>([
  "string",
  "number",
  "boolean",
  "bigint",
...
export function classifyFieldType(typeNode: ts.TypeNode | undefined, checker: ts.TypeChecker): FieldClassification {
  if (!typeNode) {
...
function classifyTypeNode(typeNode: ts.TypeNode, checker: ts.TypeChecker): FieldClassification {
...
function isBasicKeywordTypeNode(node: ts.TypeNode): boolean {
  switch (node.kind) {
...
function getTypeRefName(node: ts.TypeReferenceNode): string | undefined {
  const tn = node.typeName;
  if (ts.isIdentifier(tn)) {
    return tn.text;
  }
  // QualifiedName: Module.Name → use last segment
  let cur: ts.EntityName = tn;
