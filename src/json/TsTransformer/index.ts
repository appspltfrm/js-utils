            ...
            for (const name of names) {
                ...
                if (isDefault) {
                    ...
                    importSpecifiers.push(
                        ts.factory.createImportSpecifier(false, ts.factory.createIdentifier(name), ts.factory.createIdentifier(`_T_${name}`))
                    );
                }
            }
...
function visitNodeAndChildren(node: ts.SourceFile, program: ts.Program, context: ts.TransformationContext, typesToImport: Map<string, Set<string>>): ts.SourceFile;
function visitNodeAndChildren(node: ts.Node, program: ts.Program, context: ts.TransformationContext, typesToImport: Map<string, Set<string>>): ts.Node | undefined;
function visitNodeAndChildren(node: ts.Node, program: ts.Program, context: ts.TransformationContext, typesToImport: Map<string, Set<string>>): ts.Node | undefined {
...
function visitNode(node: ts.SourceFile, program: ts.Program, typesToImport: Map<string, Set<string>>): ts.SourceFile;
function visitNode(node: ts.Node, program: ts.Program, typesToImport: Map<string, Set<string>>): ts.Node | undefined;
function visitNode(node: ts.Node, program: ts.Program, typesToImport: Map<string, Set<string>>): ts.Node | undefined {
    const typeChecker = program.getTypeChecker();

    const symbolIsRuntimeValue = (sym: ts.Symbol): boolean => {
        ...
        if ((isInterface || isTypeAlias) && !hasValueFlag) return false;
        return hasValueFlag;
    };
    ...
    if (ts.isDecorator(node) && ts.isClassDeclaration(node.parent) && ts.isCallExpression(node.expression)) {
        ...
        if (isSerializable) {
            ...
            const isUsedAsValue = (symbol: ts.Symbol): boolean => {
                ...
                const checkNode = (n: ts.Node): boolean => {
                    if (ts.isIdentifier(n) && n.parent) {
                        ...
                        while (p && p !== currentSourceFile) {
                             if (ts.isImportClause(p) || ts.isImportSpecifier(p) || ts.isImportDeclaration(p)) return false;
                             p = p.parent;
                        }

                        const nSymbol = typeChecker.getSymbolAtLocation(n);
                        if (nSymbol) {
                             const getRootSymbol = (s: ts.Symbol): ts.Symbol => {
            ...
            const addTypeToImport = (symbol: ts.Symbol): ts.Identifier | undefined => {
                ...
                if (!symbolIsRuntimeValue(symbol)) return undefined;
            ...
            const getPropertyConfig = (typeNode: ts.TypeNode | undefined): ts.ObjectLiteralExpression => {
                ...
                if (typeNode) {
                    if (ts.isArrayTypeNode(typeNode)) {
                         ...
                         const typeIdent = config.properties.find(p => ts.isPropertyAssignment(p) && (p.name as ts.Identifier).text === "propertyType") as ts.PropertyAssignment | undefined;
                         if (typeIdent) {
            ...
            return ts.factory.updateDecorator(node,
                ts.factory.updateCallExpression(call, call.expression, undefined,
                    call.arguments.concat(
                        ts.factory.createObjectLiteralExpression(optionsProps)
                    )
                )
            );
        }
